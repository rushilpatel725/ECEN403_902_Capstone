...
void loop() {
  float leakDetected = analogRead(WATER_SENSOR_PIN);
...
void sendLeakStatus(int status) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    String payload = String("{\"leak\": ") + status + ", \"ts_ms\": {\".sv\": \"timestamp\"}}";
    int responseCode = http.POST(payload);  // Send POST request to Firebase
